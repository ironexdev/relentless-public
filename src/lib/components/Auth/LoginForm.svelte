...
>
	<MyInput
		label={t_login_form_email_label(locale)}
		name="email"
		type="email"
		autocomplete="email"
	...
	<MyButton
		type="submit"
		title={t_login_form_submit_button(locale)}
		class="w-[160px]"
		disabled={loading}
	>
		{#if loading}
		...
		{:else}
			{t_login_form_submit_button(locale)}
		{/if}
	</MyButton>
	<NavLink
