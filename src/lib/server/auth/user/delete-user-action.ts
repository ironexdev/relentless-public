...
import {
	t_user_delete_success,
	t_user_delete_invalid_or_expired_pin
...
export async function deleteUserAction(event: RequestEvent) {
	...
	const token = await db.query.verificationTokens.findFirst({
		where: and(
			...
			eq(verificationTokens.action, VerificationActionEnum.DELETE_USER),
			gte(verificationTokens.expires, new Date())
		)
	});

	if (!token) {
		return fail(422, { message: t_user_delete_invalid_or_expired_pin(locale) });
	}
	...
	CookieService.setToast(cookies, ToastEnum.SUCCESS, t_user_delete_success(locale));

	const redirectUrl = getLocalizedUrl(locale, '/');
